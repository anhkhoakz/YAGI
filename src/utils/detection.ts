...
const PROJECT_DETECTORS = [
    ...
    { files: ["build.gradle", "build.gradle.kts"], templates: ["Gradle", "Java"] },
    { files: ["requirements.txt", "pyproject.toml"], templates: ["Python"] },
...
export const detectProjectType = async (): Promise<string[]> => {
    ...
    await Promise.all(
        PROJECT_DETECTORS.map(async ({ files, templates }) => {
            const hasFile = await Promise.any(files.map(exists)).catch(() => false);
